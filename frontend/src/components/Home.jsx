  ...
  function Home() {
    ...
    const features = [
      {
        ...
        desc: 'Just paste the link of any post or video and HiveShare will fetch and save it for you.',
        src: <Bookmarks />,
      },
      {
    ...
    return (
      <>
        <div className="overflow-x-hidden [scrollbar-width:none] absolute -z-10 inset-0 h-full w-full 
          bg-[radial-gradient(circle,#73737350_1px,transparent_1px)] 
          bg-[size:10px_10px]">
          <HomeNavbar />

          {/* Hero */}
          <div className="h-[45%]  md:h-[35%] flex justify-center items-center w-full">
            <div className="text-center flex justify-center items-center gap-4 flex-col">
              <motion.h1 initial={{ opacity: 0, y: -30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6 }}
                className="text-4xl md:text-5xl font-extrabold">Welcome to HiveShare🧠</motion.h1>
              <motion.h4
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.3, duration: 0.6 }}
                className="text-gray-500 md:text-xl font-medium">
                Save, organize, and <span className="font-bold underline underline-offset-2 text-gray-900/80">rediscover</span> your favorite posts and videos <br className="hidden md:block" /> from anywhere - all in one <span className="text-green-600 font-bold font-licorice ">smart space</span>.</motion.h4>
              <motion.div className="mt-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} >
                <Link to={'sign-up'} className="py-3 px-8 rounded-2xl inset-shadow-sm/80 inset-shadow-amber-600 text-neutral-500 text-lg font-semibold hover:bg-amber-500 hover:text-white transition-all shadow-[0px_4px_16px_rgba(17,17,26,0.1),_0px_8px_24px_rgba(17,17,26,0.1),_0px_16px_56px_rgba(17,17,26,0.1)]" >Get Started </Link>
              </motion.div>
            </div>
          </div>
          ...
          {/* Testimonials */}
          <div className="flex justify-center items-center flex-col gap-8 mt-8 md:mt-4">
