...
function Login() {
  ...
  async function handleLogIn(e) {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/login`, inputValues, {
        ...
        <div className="w-[50%] flex items-center justify-center inset-shadow-sm/18 bg-gray-100/40">
          <form onSubmit={handleLogIn}>
            <div className="flex flex-col gap-4">
              ...
              <button type="submit" className="bg-sky-600  py-2 rounded-lg w-60 text-lg font-semibold text-white cursor-pointer hover:bg-sky-500 transition-all" disabled={loader ? true : false}> {!loader ? <p>Login</p> : <p className="animate-pulse [animation-duration:0.6s]">Logging In...</p>}</button>
            </div>
          </form>
